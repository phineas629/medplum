...
# Script to upgrade Medplum dependencies from v3.3.0 to v4.0.0
echo "Upgrading Medplum dependencies to v4.0.0..."

# Function to update dependencies in a package.json file
update_dependencies() {
  local package_file=$1
  local backup_file="${package_file}.bak.v4"
  
  # Create backup
  cp "$package_file" "$backup_file"
  echo "Created backup at $backup_file"
  
  # Update @medplum/* dependencies to v4.0.0
  echo "Updating Medplum dependencies in $package_file to v4.0.0..."
  ...
  jq '
    (.dependencies | 
      ...
      reduce .[] as $key (.; .dependencies[$key] = "4.0.0")
    ) |
    (.devDependencies | 
      keys | 
      map(select(startswith("@medplum/"))) | 
      reduce .[] as $key (.; .devDependencies[$key] = "4.0.0")
    )
...
# Verify we're upgrading from v3.3.0
echo "Verifying current Medplum version is v3.3.0..."
CURRENT_VERSION=$(grep -o '"@medplum/core": "[^"]*"' ./core/package.json | cut -d'"' -f4)

if [ "$CURRENT_VERSION" != "3.3.0" ]; then
  echo "Error: Current version appears to be $CURRENT_VERSION, not 3.3.0."
  echo "You must first upgrade to v3.3.0 and run data migrations before proceeding to v4.0.0."
  echo "See DATABASE-MIGRATION-GUIDE.md for details."
...
echo "All package.json files have been updated to use Medplum v4.0.0"
echo "Next steps:"
echo "1. Run 'npm install' to install the updated dependencies"
echo "2. Verify all deprecated methods have been replaced (./verify-deprecated-methods.sh)"
echo "3. Test the application to verify compatibility with v4.0.0"
echo "4. Ensure v3.3.0 data migrations have been run if self-hosting" 