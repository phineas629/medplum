...
# 1. Unit Tests
run_test "Core Unit Tests" "npx vitest run" "ehr-frontend/core" || true

# 2. API Compatibility Tests
run_test "Medplum v4.0.0 API Tests" "npx vitest run src/__tests__/MedplumApiV4.test.ts" "ehr-frontend/core" || true

# 3. TypeScript Compilation
run_test "TypeScript Compilation Check" "tsc --noEmit" "ehr-frontend/core" || true

# 4. Dependency Checks
run_test "Dependency Check" "npm list @medplum/core @medplum/react @medplum/fhirtypes" "ehr-frontend/core" || true

# 5. Environment Variables
echo "### Environment Variables" >> ${REPORT_FILE}
echo '```' >> ${REPORT_FILE}
echo "Checking for required environment variables in code..." >> ${REPORT_FILE}
grep -r "import.meta.env" --include="*.ts" --include="*.tsx" ehr-frontend/core/src >> ${REPORT_FILE} 2>&1 || true
echo '```' >> ${REPORT_FILE}
echo -e "\n" >> ${REPORT_FILE}

# 6. Check for Deprecated Methods
if [ -f "./ehr-frontend/verify-deprecated-methods.sh" ]; then
  run_test "Deprecated Methods Check" "./verify-deprecated-methods.sh" "." || true
fi
